...
// Keys gets all keys using a pattern.
...
func (l *RedisLock) Acquire(ctx context.Context, key string, opts RedisLockOptions) error {
	...
	lock, err := l.locker.Obtain(ctx, key, time.Duration(opts.TtlS)*time.Second, &redislock.Options{
...
// ToStruct copies the result of HGetAll to a provided struct.
...
// ToSlice flattens a struct using its field tags so it can be used by HSet.
// Struct fields must have the redis tag on them otherwise they will be ignored.
