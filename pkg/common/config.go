...
// It then loads the configuration from the /etc/airstore.d/ directory and
...
func NewConfigManager[T any]() (*ConfigManager[T], error) {
	...
	// Attempt to load configs from /etc/airstore.d/
	for ext := range parserMap {
		if matches, err := filepath.Glob(fmt.Sprintf("/etc/airstore.d/*%s", ext)); err == nil {
