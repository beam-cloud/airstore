...
func (s *Scheduler) buildScalerConfig(poolName string, poolConfig types.WorkerPoolConfig) PoolScalerConfig {
	return PoolScalerConfig{
		...
		GatewayPort:               s.config.Gateway.GRPC.Port,
		GatewayExternalGRPCAddr:   s.config.Scheduler.GatewayExternalGRPCAddr,
		UseGatewayServiceHostname: s.config.Scheduler.UseGatewayServiceHostname,
		WorkerServiceAccountName:  s.config.Scheduler.WorkerServiceAccountName,
		WorkerHostNetwork:         s.config.Scheduler.WorkerHostNetwork,
		WorkerImagePullSecrets:    s.config.Scheduler.WorkerImagePullSecrets,
		RuntimeClassName:          poolConfig.RuntimeClassName,
		NodeSelector:              poolConfig.NodeSelector,
